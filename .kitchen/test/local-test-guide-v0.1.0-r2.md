...
| 1.5 | Watch the status bar | The status bar transitions through `$(loading~spin) Loading…` and lands on the success state: `$(check) 5h: NN% · 7d: NN%` (round-1 D-2 fix — the refresh fires automatically) | ☐ | |
| 1.6 | Click the status bar | The modal opens showing 5-Hour Limit and Weekly Limit blocks (and Credits Balance if displayMode is `both`). **With `modalLocation: active` (default),** the modal opens in the active editor column. **With `modalLocation: auxiliary`,** the modal opens in the secondary side bar (you may need to open the secondary side bar via `View → Appearance → Secondary Side Bar` if it isn't already visible). | ☐ | |
...
| --- | --- | --- | --- | --- |
| 3.1 | **A — Invalid key**: set a wrong key in `SecretStorage` (use `code --user-data-dir /tmp/vscode-test-1` for a clean profile) | Status bar: `$(error) Sign in` (red). Modal: red left-border error block, title "Couldn't verify your Token Plan key", body distinguishes Subscription Key vs Open Platform API Key AND mentions the region hint. Recovery: Open Settings. | ☐ | |
| 3.2 | **B — Expired key**: use a real but revoked key (or a subscription that has ended) | Same UX as 3.1. The wire does not distinguish "expired" from "invalid". | ☐ | |
...
| 3.6 | **F — 5-hour at 0%**: wait until the 5-hour quota is exhausted (or use a test profile on a fresh plan). | Status bar: `$(warning) 5h: 0%` (yellow). Modal: 5-Hour bar at 0%, "Quota used 100%", "Resets in 1h 02m". The Weekly block continues to render normally. The error block is NOT shown — this is a normal state, not an error. | ☐ | |
...
| 4.5 | Test the `modalLocation: auxiliary` setting: open the side bar, click the status bar, observe the modal in the secondary side bar | The modal opens in the secondary side bar. The polling controller still refreshes the data (visible in the Output panel). | ☐ | |
...
| # | Action | Expected result | Pass/Fail | Notes |
| --- | --- | --- | --- | --- |
| 6.1 | From `v0.1.0`, run `npm run build` | Two bundles produced: `dist/extension.js` (~50kb) + `dist/webview/modal.js` (~10kb). Sourcemaps present. The new esbuild `assertWebviewAssets()` check passes silently. | ☐ | |
| 6.2 | Run `npm run package:prod` (no env var) | `minimax-usage-0.1.0.vsix` produced. Exit 0. **No `vsce` warning about `*` activation** (round-1 D-5 fix). | ☐ | |
...
| 6.4 | Verify the icon is in the .vsix | `unzip -l minimax-usage-0.1.0.vsix \| grep logo` shows `extension/images/logo-v0.1.0.png` | ☐ | |
